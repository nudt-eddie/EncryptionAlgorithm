...
def sha512(message):
    ...
    while (len(message) + 16) % 128 != 0:
        message.append(0x00)
    message += struct.pack('>Q', 0) + struct.pack('>Q', length)
    ...
    for i in range(0, len(message), 128):
        ...
        w = [0] * 80
        for j in range(16):
            w[j] = struct.unpack('>Q', chunk[j*8:j*8+8])[0]
        for j in range(16, 80):
            s0 = (w[j-15] >> 1 | w[j-15] << 63) ^ (w[j-15] >> 8 | w[j-15] << 56) ^ (w[j-15] >> 7)
            s1 = (w[j-2] >> 19 | w[j-2] << 45) ^ (w[j-2] >> 61 | w[j-2] << 3) ^ (w[j-2] >> 6)
...
if __name__ == "__main__":
    ...
    print(f"SHA-256 Hash: {sha256(message)}")
    print(f"SHA-512 Hash: {sha512(message)}")
